class FermentableAdditionsController < ApplicationController
  ...
  def index
    @fermentable_additions = FermentableAddition.includes(:recipe, :fermentable).all
  end
  ...
  private
    ...
    def set_fermentable_addition
      @fermentable_addition = FermentableAddition.includes(:recipe, :fermentable).find(params[:id])
    end

    # Only allow a list of trusted parameters through.
    def fermentable_addition_params
      params.require(:fermentable_addition).permit(:recipe_id, :fermentable_id, :amount, :time_unit, :time, :duration_unit, :duration, :step, :use)
    end
end
